...
void Model::loadMesh(unsigned int indMesh)
{
	...
	std::vector<Vertex> verticies = assembleVerticies(positions, normals, texUVs);
	std::vector<GLuint> indicies = getIndicies(JSON["accessors"][indAccInd]);
	std::vector<Texture> textures = getTextures();

	// Combine the vertices, indices, and textures into a mesh
	meshes.push_back(Mesh(verticies, indicies, textures, GL_TRIANGLES));
}
...
std::vector<Texture> Model::getTextures()
{
	...
	for (unsigned int i = 0; i < JSON["images"].size(); i++)
	{
		...
		if (!skip)
		{
			...
			{
				Texture diffuse = Texture((fileDirectory + texPath).c_str(), "diffuse", GL_TEXTURE_2D, loadedTex.size());
...
std::vector<Vertex> Model::assembleVerticies
(
